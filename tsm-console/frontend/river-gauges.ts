...
export const ACTIVE_RIVER_GAUGES: readonly Omit<RiverGauge, 'state' | 'observedAt' | 'retrievedAt' | 'stageFt' | 'dischargeCfs' | 'provisional' | 'cacheAgeSeconds'>[] = [
...
export function transitionGauge(current: RiverGauge, event: GaugeTransition): RiverGauge {
  switch (event.type) {
    case 'LIVE_OBSERVATION':
      if (!validTimestamp(event.observedAt)) throw new TypeError('invalid live observation timestamp');
      return { ...current, state: 'LIVE', observedAt: event.observedAt, retrievedAt: new Date().toISOString(), stageFt: finiteGaugeValue(event.stageFt, 'live stage'), dischargeCfs: finiteGaugeValue(event.dischargeCfs, 'live discharge'), provisional: event.provisional, cacheAgeSeconds: 0 };
    case 'STALE_CACHE':
      if (!validTimestamp(event.observedAt)) throw new TypeError('invalid stale observation timestamp');
      if (!Number.isFinite(event.cacheAgeSeconds) || event.cacheAgeSeconds < 0) throw new TypeError('invalid cache age');
      return { ...current, state: 'STALE', observedAt: event.observedAt, stageFt: finiteGaugeValue(event.stageFt, 'stale stage'), dischargeCfs: finiteGaugeValue(event.dischargeCfs, 'stale discharge'), cacheAgeSeconds: event.cacheAgeSeconds };
    case 'SOURCE_FAILURE':
      return { ...current, state: 'SOURCE_UNAVAILABLE', observedAt: null, retrievedAt: null, stageFt: null, dischargeCfs: null, provisional: false, cacheAgeSeconds: null };
  }
}

export function createUnavailableGauge(config: typeof ACTIVE_RIVER_GAUGES[number]): RiverGauge {
  return { ...config, state: 'SOURCE_UNAVAILABLE', observedAt: null, retrievedAt: null, stageFt: null, dischargeCfs: null, provisional: false, cacheAgeSeconds: null };
}
