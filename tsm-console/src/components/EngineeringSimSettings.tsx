...
export function EngineeringSimSettings() {
  ...
  const formatRuleValue = (value: number | undefined, digits: number, unit = '') => value == null ? 'not specified' : `${value.toFixed(digits)}${unit}`;
  return (
    <div style={panel}>
      ...
      {masterOpen && <>
        <Section title="Community scenario inputs"><div>BFE {VIEWPORT_CONFIG.elevations.bfeFt ?? 'not verified'} ft NAVD88 — source evidence required</div><div>LAG {VIEWPORT_CONFIG.elevations.lagFt ?? 'not verified'} ft NAVD88</div><div>FFE {VIEWPORT_CONFIG.elevations.ffeFt ?? 'not verified'} · Berm {VIEWPORT_CONFIG.elevations.bermCrestFt ?? 'not verified'}</div><div>CRS EPSG:{VIEWPORT_CONFIG.crs.horizontalEpsg} · {VIEWPORT_CONFIG.crs.verticalDatum}</div><div>Scope: Lower Wabash–Ohio Confluence Community · no private parcel identifier</div></Section>
        ...
        <Section title="Indiana 312 IAC / compensatory (S-3)"><div>No-rise (FEMA floodway practice): {formatRuleValue(inRule.no_rise_threshold_ft, 2, ' ft')}</div><div>IDNR adverse (312 IAC 10-2-3): 0.15 ft</div><div>Compensatory ratio: <strong>{INDIANA_COMPENSATORY_STORAGE_POLICY.ratio}×</strong></div><div>Freeboard (building practice): {formatRuleValue(inRule.freeboard_req_ft, 1, ' ft')}</div><div style={{ color: '#94a3b8', marginTop: 6 }}>{INDIANA_COMPENSATORY_STORAGE_POLICY.ratio_label}</div></Section>
