...
export default function RealWorldTwinMap({ data }: RealWorldTwinMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      new maplibregl.Marker().setLngLat(NEW_HARMONY_GAGE).setPopup(new maplibregl.Popup().setHTML(`<strong>USGS 03378500 / NOAA NHRI3</strong><br>Stage: ${data.stage.value_ft == null ? 'unavailable' : `${data.stage.value_ft.toFixed(2)} ft`} ${data.stage.qualifier ? `(${data.stage.qualifier})` : ''}<br>WSE NAVD88: ${data.stage.wse_navd88_ft == null ? 'unavailable' : `${data.stage.wse_navd88_ft.toFixed(2)} ft`}<br>Discharge: ${data.stage.discharge_cfs == null ? 'unavailable' : `${data.stage.discharge_cfs.toLocaleString()} cfs`}<br>Observed: ${data.stage.timestamp || 'unavailable'}`).addTo(map);
      mapRef.current = map;
    });
  ...
  return (
    <section aria-label="Real-source Indiana open-world digital twin" style={{ position: 'relative', height: '100%', minHeight: 480, background: '#020617' }}>
      ...
      <div style={{ position: 'absolute', left: 12, top: 12, zIndex: 2, maxWidth: 500, padding: 12, borderRadius: 10, background: 'rgba(2,6,23,0.9)', color: '#e2e8f0', fontSize: 12, lineHeight: 1.5 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}><button type="button" aria-pressed={femaVisible} onClick={() => setFemaVisible((value) => !value)}>FEMA NFHL {femaVisible ? 'ON' : 'OFF'}</button><button type="button" aria-pressed={bafmVisible} onClick={() => setBafmVisible((value) => !value)}>Indiana BAFM {bafmVisible ? 'ON' : 'OFF'}</button></div>
