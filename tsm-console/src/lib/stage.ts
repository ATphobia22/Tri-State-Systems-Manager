...
export async function fetchLiveStage(): Promise<MapTwinLoaderData['stage']> {
  try {
    const response = await fetch('/api/hydrologic/live?source=auto&usgs_id=03378500&nws_id=NHRI3', { headers: { Accept: 'application/json' }, cache: 'no-store' });
    ...
    return {
      ...
      gaugeId: typeof record.gaugeId === 'string' ? record.gaugeId : '03378500',
      value_ft: value,
