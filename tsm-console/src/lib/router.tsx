...
let contracts: DataContractSummary[] = [
  { id: 'tsm-hydro-001', title: 'Tri-State River Stage Observations', owner: 'Indiana DNR / USGS Water Data', classification: 'restricted', jurisdiction: 'Indiana', validation_status: 'validated', content_hash: 'sha256:pending' },
  ...
  { id: 'tsm-site-001', title: '13101 Bonebank Site Constants', owner: 'TSM Project Evidence', classification: 'restricted', jurisdiction: 'Indiana', validation_status: 'validated', content_hash: 'sha256:site-v1' },
];

async function rootLoader(): Promise<RootLoaderData> {
  const [auth, stage] = await Promise.all([authLoader({}), fetchLiveStage()]);
  return { auth, stage, systemClock: new Date().toISOString(), siteSummary: { address: SITE.address, apn: SITE.apn, bfe: SITE.elevations.bfe_ft, lag: SITE.elevations.lag_ft, clearanceAboveBfe: SITE.elevations.clearanceAboveBfe_ft } };
}
