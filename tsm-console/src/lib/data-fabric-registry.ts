...
export const TSM_DATA_FABRICS: readonly TsmDataFabric[] = Object.freeze([
  ...
  { id:'bathymetry-topobathy', plane:'EVIDENCE_DATA_GOVERNANCE', authority:'AUTHORITATIVE', implemented:true, live:false, persistenceAllowed:true, module:'engineering/bathymetry evidence contracts + geospatial ingestion', sources:['USGS-3DEP-TOPOBATHY','surveyed cross-sections','project bathymetric survey'], output:['channel bed surface','cross-sections','topobathymetric evidence'], notes:'Required for submerged-channel modeling; LiDAR/DEM water surfaces are not treated as channel-bed truth.' },
  { id:'terrain-elevation', plane:'EVIDENCE_DATA_GOVERNANCE', authority:'AUTHORITATIVE', implemented:true, live:true, persistenceAllowed:true, module:'lib/open-world-terrain.ts + lib/elevationModel.ts', sources:['USGS-3DEP-TERRAIN','INDIANA-ELEVATION-2016-2020','INDIANA_STATEWIDE_ELEVATION_CATALOG','AWS_OPEN_DATA_REGISTRY'], output:['DEM','terrain RGB','terrain mesh'], notes:'Horizontal CRS and vertical datum remain independent metadata; raw LiDAR remains an elevation archive and never becomes survey/FEMA certification.' },
  ...
  { id:'engineering-evidence-pipeline', plane:'GOVERNANCE_DECISION', authority:'DERIVED', implemented:true, live:false, persistenceAllowed:true, module:'data/engineering/evidence-pipeline-contract.json + scripts/ci/validate-engineering-pipeline.mjs', sources:['engineering evidence artifacts','model validation records','agency determinations'], output:['stage gates','blocking reasons','provenance graph'], notes:'Fail-closed sequence from authoritative terrain through QA/QC; no downstream stage may outrank an unverified upstream stage.' },
