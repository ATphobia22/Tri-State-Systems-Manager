...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/auth/health') return json(res, 200, { ok: true, service: 'tsm-api', auth_model: 'keycloak_public_client_pkce', client_secret_required: false, token_proxy_path: null, planes: ['EVIDENCE', 'GOVERNANCE', 'ENGINEERING', 'GEOSPATIAL', 'DATA_FABRIC'] });
    ...
    if (req.method === 'GET' && url.pathname === '/api/data-sources/health') return json(res, 200, { sources: listSourceHealth() });
  ...
  } catch (error) { return json(res, 502, { error: error.message || 'upstream source unavailable', code: 'SOURCE_UNAVAILABLE' }); }
});
