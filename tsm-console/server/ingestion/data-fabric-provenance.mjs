...
export function validateProvenanceContract(provenance) {
  ...
  if (!AUTHORITY_CLASSES.has(authorityClass)) throw Object.assign(new TypeError('unsupported authority_class'), { code: 'PROVENANCE_CONTRACT_INVALID' });
  const governanceStatus = requiredString(provenance.governance_status || 'public', 'governance_status');
  if (!GOVERNANCE_STATUSES.has(governanceStatus)) throw Object.assign(new TypeError('unsupported governance_status'), { code: 'PROVENANCE_CONTRACT_INVALID' });
...
export function buildProvenanceManifest({ datasetId, payload, provenance }) {
  ...
  return Object.freeze({
    dataset_id: requiredString(datasetId, 'datasetId'),
