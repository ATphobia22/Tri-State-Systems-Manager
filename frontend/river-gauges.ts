...
export function transitionGauge(
  ...
  switch (event.type) {
    case 'LIVE_OBSERVATION':
      if (!Number.isFinite(event.stageFt ?? 0) && event.stageFt !== null) throw new TypeError('invalid live stage');
      return { ...current, state: 'LIVE', observedAt: event.observedAt, retrievedAt: new Date().toISOString(), stageFt: event.stageFt, dischargeCfs: event.dischargeCfs, provisional: event.provisional, cacheAgeSeconds: 0 };
    case 'STALE_CACHE':
      if (!Number.isFinite(event.cacheAgeSeconds) || event.cacheAgeSeconds < 0) throw new TypeError('invalid cache age');
      return { ...current, state: 'STALE', observedAt: event.observedAt, stageFt: event.stageFt, dischargeCfs: event.dischargeCfs, cacheAgeSeconds: event.cacheAgeSeconds };
    case 'SOURCE_FAILURE':
