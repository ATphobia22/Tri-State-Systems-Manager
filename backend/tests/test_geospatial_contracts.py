...
def test_indiana_map_contract_requires_https_and_authority_metadata() -> None:
    ...
    assert record.dataset_id == "Indiana_2016_2020_DEM"


def test_indiana_map_contract_rejects_untrusted_service_type() -> None:
...
def test_temporal_provenance_normalizes_to_utc() -> None:
    result = TemporalProvenance(
        datetime(2026, 9, 19, 1, tzinfo=timezone.utc),
        datetime(2026, 9, 19, 2, tzinfo=timezone.utc),
    )
    assert result.valid_time.tzinfo == timezone.utc


def test_temporal_provenance_rejects_future_change_time() -> None:
