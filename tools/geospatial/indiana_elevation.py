...
This client deliberately treats the service as raster data, never as vector
terrain or synthetic elevation. It requests raw floating-point pixels from
ArcGIS ImageServer/exportImage and records the service metadata used for the
request.

The vertical datum is NOT inferred from the service name. Callers must
explicitly provide --vertical-datum only when an authoritative source has
verified the datum for the requested service/product.
...
The output GeoTIFF is requested in EPSG:2966 so the TSM horizontal CRS is
preserved without a client-side approximation. ArcGIS documents bboxSR and
imageSR as the coordinate systems for the request and output respectively.
"""
...
def http_json(url: str, timeout: int) -> dict[str, Any]:
    request = Request(
        url,
        headers={"Accept": "application/json", "User-Agent": "TSM-Indiana-Elevation/1.0"},
    )
    try:
...
def download_raster(url: str, output: Path, timeout: int) -> None:
    ...
    request = Request(
        url,
        headers={"Accept": "image/tiff", "User-Agent": "TSM-Indiana-Elevation/1.0"},
    )
    try:
        with urlopen(request, timeout=timeout) as response, temporary.open("wb") as stream:
            ...
            while True:
                ...
                if total > 512 * 1024 * 1024:
                    raise RuntimeError("Downloaded raster exceeded the 512 MiB safety limit.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--bbox-sr", type=int, default=4326, help="WKID of --bbox (default: 4326)")
