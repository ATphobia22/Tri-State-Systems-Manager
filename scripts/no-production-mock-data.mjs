...
const PROHIBITED = [/\bmock\b/i, /\bfake\b/i, /\bsynthetic\b/i, /\bdemo\b/i, /Math\.random\s*\(/];
...
function collectFiles(root) {
  ...
  function walk(directory) {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      else if (/\.(mjs|js|ts|tsx|jsx|json|html)$/.test(entry.name)) output.set(relative, fs.readFileSync(absolute, 'utf8'));
    }
  }
  walk(root);
  return output;
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(import.meta.url.slice(7))) {
  ...
  else console.log('No production mock/synthetic markers detected.');
}
