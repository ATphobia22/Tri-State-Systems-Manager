...
-- A FIRM source record may exist without georeferencing; derivatives require
...
BEGIN
    IF NEW.public_release THEN
        ...
        IF parent_status <> 'validated' THEN
            RAISE EXCEPTION
                'fail-closed: FIRM panel % is not validated',
                NEW.panel_id;
        END IF;

        IF NEW.validation_status <> 'validated' THEN
            RAISE EXCEPTION
                'fail-closed: FIRM derivative % is not validated',
                NEW.derivative_id;
        END IF;
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION prevent_unvalidated_firm_public_release();
